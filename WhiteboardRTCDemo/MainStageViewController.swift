...
//        agoraKit.setParameters("\"che.audio.use_ffmpeg_audio_decoder\": true")
        agoraKit.joinChannel(byToken: joinInfo.rtcToken, channelId: joinInfo.roomUUID, info: nil, uid: UInt(joinInfo.rtcUID)) { [weak self] _, _, elapsed in
            ...
            //                self.agoraKit.startAudioMixing(src, loopback: false, replace: false, cycle: 1)
            //                DispatchQueue.main.asyncAfter(deadline: .now() + 3) {
            //                    print("/State/ start set effect position")
            ////                    self.agoraKit.setEffectPosition(999, pos: 3000000)
            //                    self.agoraKit.setAudioMixingPosition(3000000)
            //                }
            //            }
        }
...
extension MainStageViewController: WhiteAudioEffectMixerBridgeDelegate {
    ...
    func playEffect(_ soundId: Int32, filePath: String?, loopCount: Int32, pitch: Double, pan: Double, gain: Double, publish: Bool, startPos _: Int32) -> Int32 {
